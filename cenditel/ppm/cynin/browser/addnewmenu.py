...
from cenditel.ppm.config import spacesdefaultaddableforfolders
...
from ubify.viewlets.browser.addnewmenu import AddNewMenuViewlet 

class AddNewMenuViewletPpm(AddNewMenuViewlet):
    ...
    def update(self):
        ...
        if not self.anonymous:
            ...
            if self.context.portal_type == 'folderproj':
                self.addnewitems = []
            ...
            containers = ['ContentSpace', 'folderproj']
            ...
            if object_typename in ('RecycleBin',):
                ...
                if object_typename in ('ContentRoot','ContentSpace', 'folderproj') and self.context.isPrincipiaFolderish and checkHasPermission('Add portal content',aq_inner(self.context)):                
                    self.currentcontextmenu = menu.getMenuItems(self.context, self.request)
                    self.contextualurl = aq_inner(self.context).absolute_url()
                    
                    if object_typename in ('ContentRoot','ContentSpace','folderproj'):
                    ...
                    try:
                        for type in parentList:
                            if type.portal_type in ('ContentRoot','ContentSpace','folderproj'):
                                parentspace = type
